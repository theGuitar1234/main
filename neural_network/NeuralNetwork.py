...
class NeuralNetwork:
    ...
    def train(self, X, Y, X_val, Y_val, learning_decay_type=None, data_augmentation_type=None, cfg=None, log=True, graph=True):
        if cfg is None:
            cfg = self.TrainDefaults()
        ...
        for epoch in range(epochs):
            ...
            train_total_loss = train_data_loss + train_reg_loss
            _, val_loss, val_acc = self.evaluate_dataset(X_val, Y_val)
...
if __name__ == "__main__":
    ...
    model.train(X_train_small, Y_train_small, X_valid, Y_valid_one_hot)
